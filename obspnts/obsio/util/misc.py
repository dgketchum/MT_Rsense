...
class StatusCheck(object):
    ...
    def increment(self, n=1):
        ...
        if self.num - self.num_last_check >= self.check_cnt:
            ...
            if self.total_cnt != -1:
                print "Total items processed is %d.  Last %d items took %f minutes. %d items to go." % (
                self.num, self.num - self.num_last_check, (currentTime - self.status_time) / 60.0,
                ...
                ((self.total_cnt - self.num) / float(self.num)) * ((currentTime - self.start_time) / 60.0))

            else:
                print "Total items processed is %d.  Last %d items took %f minutes" % (
                self.num, self.num - self.num_last_check, (currentTime - self.status_time) / 60.0)
                print "Current total process time: %f minutes" % ((currentTime - self.start_time) / 60.0)
            sys.stdout.flush()
            self.status_time = time.time()
